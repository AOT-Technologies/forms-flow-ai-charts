...
This chart bootstraps a forms-flow-data-layer deployment on a [Kubernetes](https://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.


## Installing the Chart

To install the chart with the release name `forms-flow-data-layer`:

```console
helm install forms-flow-api forms-flow-data-layer
```
...
```console
helm install forms-flow-data-layer forms-flow-data-layer  --set ingress.ingressClassName=INGRESS_CLASS  --set ingress.hostname=HOSTNAME
```
...
By default, the chart uses the OpenSource version of the `Forms-flow Data Layer`. You can change the image tag to any valid version of the OpenSource image, as shown below:
...
If you're using the enterprise version of Forms-flow API, you can switch the image repository to `formsflow/forms-flow-data-layer-ee`. The enterprise version includes additional features and support designed for larger-scale or production environments. To use the enterprise version, update the repository field as shown below:

```yaml
image:
  registry: docker.io
  repository: formsflow/forms-flow-data-layer-ee
...
The `form-flow-data-layer` image stores the application logs at the `/form-flow-data-layer/app/logs` path of the container.

## Sidecar Configuration

To add a sidecar to your `form-flow-data-layer` deployment, you can use the following configuration. In this case, the sidecar container is an Nginx container used for configuration management.
...
## API Path Update
The `form-flow-data-layer` can now be accessed at the `/datalayer` route. Ensure that all configurations and requests reference this updated path.

For example:

```
https://<HOSTNAME>/datalayer
```
## Parameters
...
| `autoscaling.enabled`           | Enable autoscaling for form-flow-data-layer                                                                | `false`                     |
| `autoscaling.minReplicas`       | Minimum number of form-flow-data-layer replicas                                                            | `1`                         |
| `autoscaling.maxReplicas`       | Maximum number of form-flow-data-layer replicas                                                            | `11`                        |
...
| `formsflow.bpm.configmap`           | Name of formsflow bpm configmap                                                                       | `forms-flow-bpm`             |
| `formsflow.bpm.secret`              | Name of formsflow bpm  secret                                                                          | `forms-flow-bpm`             |
| `formsflow.webapi.configmap`           | Name of formsflow api configmap                                                                       | `forms-flow-api`             |
| `formsflow.webapi.secret`              | Name of formsflow api secret                                                                          | `forms-flow-api`             |
| `Loglevel.Debug.enabled`           | Log level of data layer                                                                       | `false`             |
| `CorsOrigins`              | Cors origin of datalayer                                                                         | `*`             |
| `WorkerLayer`              | Worker layer of datalayer                                                                         | `4`             |
## Ingress Parameters
...
| `ingress.hostname`             | Default host for the ingress record                                                                  | `form-flow-data-layer.local`     |
| `ingress.path`                 | Default path for the ingress record                                                                   | `"/api"`                       |
| `ingress.servicePort`          | Backend service port to use (default is http)                                                       | `8000`                      |
...
| `service.ports`                 | Ports for the form-flow-data-layer service                                                                  | `[ { name: http, port: 8000, targetPort: http, protocol: TCP } ]` |
...
| `service.annotations`            | Annotations for the form-flow-data-layer service                                                           | `{}`                        |
...
| `containerPorts.http`            | HTTP port for the container                                                                           | `8000`                      |
