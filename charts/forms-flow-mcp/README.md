...
formsflow.ai has built this designed to seamlessly integrate with forms-flow components, enabling efficient communication between MCP clients and backend services. Built using FastAPI with SSE support, it provides scalable, real-time interactions while incorporating project-specific configurations, security enhancements, and optimized dependencies.


## Introduction

This chart bootstraps a forms-flow-mcp deployment on a [Kubernetes](https://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.


## Installing the Chart

To install the chart with the release name `forms-flow-mcp`:

```console
helm install forms-flow-mcp forms-flow-mcp
```
...
```console
helm install forms-flow-mcp forms-flow-mcp  --set ingress.ingressClassName=INGRESS_CLASS  --set ingress.hostname=HOSTNAME
```

> Note: You need to substitute the placeholders `INGRESS_CLASS` and `HOSTNAME` with a reference to your Helm chart registry and repository. For example, in the case of Formsflow, you need to use `INGRESS_CLASS=nginx`. Use  `--set image.repository=formsflow/forms-flow-mcp` for deploy the enterprise version

These commands deploy forms-flow-mcp on the Kubernetes cluster

> **Tip**: List all releases using `helm list`

### Resource requests and limits

forms-flow-mcp charts allow setting resource requests and limits for all containers inside the chart deployment. These are inside the `resources` value (check parameter table). Setting requests is essential for production workloads and these should be adapted to your specific use case.
...
### Change forms-flow-mcp version

To modify the forms-flow-mcp version used in this chart you can specify a [valid image tag](https://hub.docker.com/repository/docker/formsflow/forms-flow-mcp) using the `image.tag` parameter. For example, `image.tag=X.Y.Z`. This approach is also applicable to other images like exporters.
...
image:
  registry: docker.io
  repository: formsflow/forms-flow-mcp
  tag: X.Y.Z # Replace with the desired OpenSource version
```
#### Using the Enterprise Version
If you're using the enterprise version of Forms-flow API, you can switch the image repository to `formsflow/forms-flow-mcp`. The enterprise version includes additional features and support designed for larger-scale or production environments. To use the enterprise version, update the repository field as shown below:
...
The `forms-flow-mcp` image stores the application logs at the `/forms-flow-mcp/app/logs` path of the container.

## Sidecar Configuration

To add a sidecar to your `forms-flow-mcp` deployment, you can use the following configuration. In this case, the sidecar container is an Nginx container used for configuration management.
...
## API Path Update
The `forms-flow-mcp` can now be accessed at the `/mcp` route. Ensure that all configurations and requests reference this updated path.
...
| `image.repository`                                   | Repository for the image                            | `formsflow/forms-flow-mcp` |
...
| `autoscaling.enabled`           | Enable autoscaling for forms-flow-mcp                                                                | `false`                     |
| `autoscaling.minReplicas`       | Minimum number of forms-flow-mcp replicas                                                            | `1`                         |
| `autoscaling.maxReplicas`       | Maximum number of forms-flow-mcp replicas                                                            | `11`                        |
...
| `ingress.hostname`             | Default host for the ingress record                                                                  | `forms-flow-mcp.local`     |
...
| `service.ports`                 | Ports for the forms-flow-mcp service                                                                  | `[ { name: http, port: 5000, targetPort: http, protocol: TCP } ]` |
...
| `service.annotations`            | Annotations for the forms-flow-mcp service                                                           | `{}`                        |
